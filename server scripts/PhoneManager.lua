...
📌 Возможно, внутри здания ты найдёшь важные данные, которые помогут понять, что случилось с Платформой.]], 
		tasks = [[Задача: Разблокируй дверь одного из пунктов. Надо проверить, закрыта ли дверь. Если дверь закрыта, тогда ее надо открыть.]]},
	["Phone_5"] = {
...
_"Если система видит, что **и гараж, и освещение выключены**, она их заблокирует. Но если они оба включены – всё должно сработать."]], 
		...
		tasks = [[
    📌 **Задача:**  
Проверь статус **гаража** и **освещения**. Если они выключены, включи их!
    ]]
	},
	["Phone_7"] = {
...
- Возможно, можно найти способ **отрегулировать напряжение** и обойти блокировку?]],
		tasks = [[📌 **Задача:**  
Надо написать код, при котором будет подано нужное количество энергии на главные ворота.
...
Будь осторожен: Платформа следит за тобой. Я оставил подсказки в терминалах. Ищи их."]],
		tasks = "" -- Нет задачи, только лор
	},
	["Phone_10"] = {
		title = "🔑 Второй этаж: Код доступа",
		content = "", -- Без лишнего лора
		tasks = [[📌 **Задача:**  
...
-- Функция обработки взаимодействия с телефоном
local function onPhoneActivated(player, phone)
	if phoneData[phone.Name] then
		print("Отправляем данные телефона " .. phone.Name .. ": title=" .. phoneData[phone.Name].title .. ", content=" .. phoneData[phone.Name].content .. ", tasks=" .. phoneData[phone.Name].tasks)
		phoneEvent:FireClient(player, phoneData[phone.Name].title, phoneData[phone.Name].content, phoneData[phone.Name].tasks)
		phone:Destroy()
	end
end

-- Подключаем обработчики ко всем телефонам
for _, phone in pairs(workspace.level_1.interactive:GetChildren()) do
	if phone:IsA("Part") and phone:FindFirstChild("ProximityPrompt") then
		phone.ProximityPrompt.Triggered:Connect(function(player)
			onPhoneActivated(player, phone)
		end)
	end
end

for _, phone in pairs(workspace.level_2.interactive:GetChildren()) do
	if phone:IsA("Part") and phone:FindFirstChild("ProximityPrompt") then
		phone.ProximityPrompt.Triggered:Connect(function(player)
			onPhoneActivated(player, phone)
		end)
...
for _, phone in pairs(workspace.level_3.interactive:GetChildren()) do
	if phone:IsA("Part") and phone:FindFirstChild("ProximityPrompt") then
		phone.ProximityPrompt.Triggered:Connect(function(player)
			onPhoneActivated(player, phone)
		end)
...
for _, phone in pairs(workspace.level_4.interactive:GetChildren()) do
	if phone:IsA("Part") and phone:FindFirstChild("ProximityPrompt") then
		phone.ProximityPrompt.Triggered:Connect(function(player)
			onPhoneActivated(player, phone)
		end)
